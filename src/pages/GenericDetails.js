...
const GenericDetails = () => {
    ...
    }, [gameData, alternateRegion, playerData, timelineData]);
                        ...
                        <Grid style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', margin: 'auto' }} item xs={7}>
                            ...
                            <Grid>
                                <Typography style={{ paddingTop: '10px' }} fontSize={26} fontWeight={600}>
                                    ...
                                    <span style={{ color: 'black' }}>{' remade'}</span> playing {Object.values(champsJSON.data).find(champ => champ.key === String(playerData.championId)).name} {playerData.teamPosition.toLowerCase()} for <span style={{ color: playerData.teamId === 100 ? '#3374FF' : '#FF3F3F' }}>{playerData.teamId === 100 ? 'blue team' : 'red team'}</span> finishing {playerData.kills}/{playerData.deaths}/{playerData.assists} with {playerData.totalMinionsKilled + playerData.neutralMinionsKilled} CS.
                                </Typography>
