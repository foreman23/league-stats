...
const LanePhaseSummaryCardJg = (props) => {
    ...
    if (statsAt15.laneResults.JUNGLE.goldDifference > 3000) {
        advantageStr = `almost guaranteeing ${statsAt15.laneResults.JUNGLE.teamWonLane === 100 ? 'blue' : 'purple'} team the victory`
    }
    else if (statsAt15.laneResults.JUNGLE.goldDifference > 2000) {
        advantageStr = `giving ${statsAt15.laneResults.JUNGLE.teamWonLane === 100 ? 'blue' : 'purple'} team a big lead entering the mid phase`
    }
    else if (statsAt15.laneResults.JUNGLE.goldDifference > 650) {
        advantageStr = `giving ${statsAt15.laneResults.JUNGLE.teamWonLane === 100 ? 'blue' : 'purple'} team an advantage entering the mid phase`
    }
    else if (statsAt15.laneResults.JUNGLE.goldDifference >= 150) {
        advantageStr = `giving ${statsAt15.laneResults.JUNGLE.teamWonLane === 100 ? 'blue' : 'purple'} team a small advantage entering the mid phase`
    }
    else if (statsAt15.laneResults.JUNGLE.goldDifference < 150) {
        ...
        <div id='laningJgAnchor'>
                ...
                <div className='LaningPhaseGridHeader'>
                    <div className='LaneOutcomeTitle'>
                        {statsAt15.laneResults.JUNGLE.resTag === 'draw' ? (
                            ...
                            <Typography fontWeight={'bold'}>{`${statsAt15.laneResults.JUNGLE.teamWonLane === 100 ? 'Blue' : 'Purple'} ${statsAt15.laneResults.JUNGLE.resTag} jungle`}</Typography>
                        )}
                    </div>
            ...
            <Grid className={lastButtonPressedJg === 'laneSumJg1' ? 'LanePhaseSummaryDetailsActive' : 'LanePhaseSummaryDetailsInActive'} style={{ flexDirection: 'row', display: 'flex' }}>
                ...
                <Grid item style={{ display: 'inline-flex', justifyContent: 'center' }} xs={12} sm={6}>
                    <StyledTooltip arrow placement='top' slotProps={{ popper: { modifiers: [{ name: 'offset', options: { offset: [0, -15] } }] } }} title={`${statsAt15.laneResults.JUNGLE.laneWinner?.riotIdGameName} #${statsAt15.laneResults.TOP.laneWinner?.riotIdTagline}`}>
                        <div style={{ position: 'relative', display: 'inline-block' }}>
                            <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${statsAt15.laneResults.JUNGLE.laneWinner?.riotIdGameName}/${statsAt15.laneResults.JUNGLE.laneWinner?.riotIdTagline.toLowerCase()}`}><img alt='Champion' style={{ border: statsAt15.laneResults.JUNGLE.laneWinner.teamId === 100 ? '4px #568CFF solid' : '4px #A35BFF solid', filter: 'drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25))' }} className='lanePhaseChampImgLarge' src={championImg(props.dataDragonVersion, winnerChampName)}></img></a>
                        </div>
                    </StyledTooltip>
                    <img alt='' className='lanePhaseSummarySwords' src='/images/swords.svg'></img>
                    <StyledTooltip arrow placement='top' slotProps={{ popper: { modifiers: [{ name: 'offset', options: { offset: [0, -15] } }] } }} title={`${statsAt15.laneResults.JUNGLE.laneLoser?.riotIdGameName} #${statsAt15.laneResults.TOP.laneLoser?.riotIdTagline}`}>
                        <div style={{ position: 'relative', display: 'inline-block' }}>
                            <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${statsAt15.laneResults.JUNGLE.laneLoser?.riotIdGameName}/${statsAt15.laneResults.JUNGLE.laneLoser?.riotIdTagline.toLowerCase()}`}><img alt='Champion' style={{ border: statsAt15.laneResults.JUNGLE.laneLoser.teamId === 100 ? '4px #568CFF solid' : '4px #A35BFF solid', filter: 'drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25))' }} className='lanePhaseChampImgLarge' src={championImg(props.dataDragonVersion, loserChampName)}></img></a>
                        </div>
                    </StyledTooltip>
            ...
            <Grid className={lastButtonPressedJg === 'laneSumJg2' ? 'LanePhaseSummaryDetailsActive' : 'LanePhaseSummaryDetailsInActive'} style={{ flexDirection: 'row', display: 'flex' }}>
                    ...
                    <div style={{ marginBottom: '10px' }}>
                        {jgLaneKillTimeline.map((kill, index) => {
                            if (kill.victimId !== 0 && kill.killerId !== 0) {
                                return (
                                    <Typography key={`kill_${index}`}>{Math.round(kill.timestamp / 60000)}m - <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${participants.find(player => player.participantId === kill.killerId)?.riotIdGameName}/${participants.find(player => player.participantId === kill.killerId)?.riotIdTagline.toLowerCase()}`} style={{ color: participants.find(player => player.participantId === kill.killerId).teamId === 100 ? '#0089D6' : '#8A3FE6', fontWeight: 'bold' }}>
                                        ...
                                        <img alt='Champion' style={{ maxWidth: '20px', maxHeight: '20px', marginLeft: '5px' }}
                                            src={championImg(props.dataDragonVersion, Object.values(champsJSON.data).find(champ => champ.key === String(participants.find(player => player.participantId === kill.killerId).championId)).id)}>
                                        </img> killed <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${participants.find(player => player.participantId === kill.victimId)?.riotIdGameName}/${participants.find(player => player.participantId === kill.victimId)?.riotIdTagline.toLowerCase()}`} style={{ color: participants.find(player => player.participantId === kill.victimId).teamId === 100 ? '#0089D6' : '#8A3FE6', fontWeight: 'bold' }}>{participants.find(player => player.participantId === kill.victimId)?.riotIdGameName}</a>
                                        <img alt='Champion' style={{ maxWidth: '20px', maxHeight: '20px', marginLeft: '5px' }}
                                            src={championImg(props.dataDragonVersion, Object.values(champsJSON.data).find(champ => champ.key === String(participants.find(player => player.participantId === kill.victimId).championId)).id)}>
                                        </img>
                                    </Typography>
                                )
                            }
                ...
                <Grid item style={{ display: 'inline-flex', justifyContent: 'center' }} xs={12} sm={6}>
                    <StyledTooltip arrow placement='top' slotProps={{ popper: { modifiers: [{ name: 'offset', options: { offset: [0, -15] } }] } }} title={`${statsAt15.laneResults.JUNGLE.laneWinner?.riotIdGameName} #${statsAt15.laneResults.JUNGLE.laneWinner?.riotIdTagline}`}>
                        <div style={{ position: 'relative', display: 'inline-block' }}>
                            <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${statsAt15.laneResults.JUNGLE.laneWinner?.riotIdGameName}/${statsAt15.laneResults.JUNGLE.laneWinner?.riotIdTagline.toLowerCase()}`}><img alt='Champion' style={{ border: statsAt15.laneResults.JUNGLE.laneWinner.teamId === 100 ? '4px #568CFF solid' : '4px #A35BFF solid', filter: 'drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25))' }} className='lanePhaseChampImgLarge' src={championImg(props.dataDragonVersion, winnerChampName)}></img></a>
                        </div>
                    </StyledTooltip>
                    <img alt='' className='lanePhaseSummarySwords' src='/images/swords.svg'></img>
                    <StyledTooltip arrow placement='top' slotProps={{ popper: { modifiers: [{ name: 'offset', options: { offset: [0, -15] } }] } }} title={`${statsAt15.laneResults.JUNGLE.laneLoser?.riotIdGameName} #${statsAt15.laneResults.JUNGLE.laneLoser?.riotIdTagline}`}>
                        <div style={{ position: 'relative', display: 'inline-block' }}>
                            <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${statsAt15.laneResults.JUNGLE.laneLoser?.riotIdGameName}/${statsAt15.laneResults.JUNGLE.laneLoser?.riotIdTagline.toLowerCase()}`}><img alt='Champion' style={{ border: statsAt15.laneResults.JUNGLE.laneLoser.teamId === 100 ? '4px #568CFF solid' : '4px #A35BFF solid', filter: 'drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25))' }} className='lanePhaseChampImgLarge' src={championImg(props.dataDragonVersion, loserChampName)}></img></a>
                        </div>
                    </StyledTooltip>
            ...
            <Grid className={lastButtonPressedJg === 'laneSumJg3' ? 'LanePhaseSummaryDetailsActive' : 'LanePhaseSummaryDetailsInActive'} style={{ flexDirection: 'row', display: 'flex' }}>
                <Grid item xs={12}>
                    <p style={{ marginBottom: '15px' }}>
                        Graph of CS killed each minute by <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${statsAt15.laneResults.JUNGLE.laneWinner?.riotIdGameName}/${statsAt15.laneResults.JUNGLE.laneWinner?.riotIdTagline.toLowerCase()}`} style={{ color: statsAt15.laneResults.JUNGLE.laneWinner.teamId === 100 ? '#0089D6' : '#8A3FE6', fontWeight: 'bold' }}> {statsAt15.laneResults.JUNGLE.laneWinner?.riotIdGameName}</a><img alt='Champion' style={{ maxWidth: '20px', maxHeight: '20px', marginLeft: '5px', marginRight: '5px' }} src={championImg(props.dataDragonVersion, winnerChampName)}></img> ({statsAt15.laneResults.JUNGLE.laneWinner.kdaAlt}, {statsAt15.laneResults.JUNGLE.laneWinner.cs} CS) and <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${statsAt15.laneResults.JUNGLE.laneLoser?.riotIdGameName}/${statsAt15.laneResults.JUNGLE.laneLoser?.riotIdTagline.toLowerCase()}`} style={{ color: statsAt15.laneResults.JUNGLE.laneLoser.teamId === 100 ? '#0089D6' : '#8A3FE6', fontWeight: 'bold' }}> {statsAt15.laneResults.JUNGLE.laneLoser?.riotIdGameName}</a><img alt='Champion' style={{ maxWidth: '20px', maxHeight: '20px', marginLeft: '5px', marginRight: '5px' }} src={championImg(props.dataDragonVersion, loserChampName)}></img> ({statsAt15.laneResults.JUNGLE.laneLoser.kdaAlt}, {statsAt15.laneResults.JUNGLE.laneLoser.cs} CS) during laning phase.
                    </p>
                            ...
                            series={[
                                {
                                    ...
                                    color: statsAt15.laneResults.JUNGLE.laneWinner.teamId === 100 ? '#37B7FF' : '#A35BFF',
                                    label: statsAt15.laneResults.JUNGLE.laneWinner?.riotIdGameName,
                                },
                                {
                                    data: yAxisDataLoser,
                                    color: statsAt15.laneResults.JUNGLE.laneLoser.teamId === 100 ? '#37B7FF' : '#A35BFF',
