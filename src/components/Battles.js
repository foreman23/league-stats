...
import clusterFightEvents from './battlesPhase/clusterFights';
...
function BattleCardList({ teamfights, graphData, ctx }) {
    ...
    // Fights are clustered by place as well as time, so two battles can share
    // a timespan. Overlapping neighbors group under a "Concurrent fights"
    // band; the cards inside stay ordinary cards (focus/scroll untouched).
    const spans = teamfights.map((f) => {
        const ds = f.details || [];
        return { s: ds[0]?.timestamp ?? 0, e: ds[ds.length - 1]?.timestamp ?? 0 };
    });
    const groups = [];
    let cur = [];
    teamfights.forEach((f, i) => {
        if (cur.length && spans[i].s <= Math.max(...cur.map((j) => spans[j].e))) {
            cur.push(i);
        } else {
    ...
    const card = (fightIndex, inBand) => (
        <BattleCard
            key={fightIndex}
            battle={toBattleVM(teamfights[fightIndex], fightIndex, graphData)}
            ctx={ctx}
            defaultOpen={fightIndex === 0}
            // the whole-map gold swing can't be split between simultaneous
            // fights, so the per-card chip hides inside a band
            hideGold={inBand}
    ...
    return (
        <>
            {groups.map((g) => (
                g.length === 1 ? card(g[0], false) : (
                    <div key={'band-' + g[0]} className="bcr-band">
                        <div className="bcr-band-head">
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M8 3H5a2 2 0 00-2 2v3M16 3h3a2 2 0 012 2v3M8 21H5a2 2 0 01-2-2v-3M16 21h3a2 2 0 002-2v-3" /></svg>
                            {g.length} concurrent fights · {clock(Math.min(...g.map((j) => spans[j].s)))} – {clock(Math.max(...g.map((j) => spans[j].e)))}
                        </div>
                        {g.map((i) => card(i, true))}
                    </div>
                )
...
const Battles = (props) => {
    ...
    // Kill events pre-clustered into fights by time + map proximity + shared
    // participants (see clusterFights.js) — the old flat 20s chaining merged
    // simultaneous fights on opposite sides of the map. The loop below is
    // unchanged except that it flushes a battle when the fight id changes.
    const clusteredEvents = clusterFightEvents(frames);
    let currFight = null;

    for (const index in clusteredEvents) {
        const currEvent = clusteredEvents[index];
        if (lastKillTime === null) {
            lastKillTime = currEvent.timestamp;
            currFight = currEvent.__fight;
            currBattle.push(currEvent);
        }
        else if (currEvent.__fight === currFight) { // same pre-clustered fight
            lastKillTime = currEvent.timestamp;
            currBattle.push(currEvent);
        }
