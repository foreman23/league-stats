        ...
        <div id='laningBotAnchor'>
                ...
                <Grid item style={{ display: 'inline-flex', justifyContent: 'center' }} xs={12} sm={6}>
                    <Tooltip arrow placement='top' slotProps={{ popper: { modifiers: [{ name: 'offset', options: { offset: [0, -20] } }] } }} title={`${statsAt15.laneResults.BOTTOM.laneWinner[0]?.riotIdGameName} #${statsAt15.laneResults.BOTTOM.laneWinner[0]?.riotIdTagline}`}>
                        <div style={{ position: 'relative', display: 'inline-block' }}>
                            <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${statsAt15.laneResults.BOTTOM.laneWinner[0]?.riotIdGameName}/${statsAt15.laneResults.BOTTOM.laneWinner[0]?.riotIdTagline.toLowerCase()}`}><img style={{ border: statsAt15.laneResults.BOTTOM.laneWinner[0].teamId === 100 ? '4px #568CFF solid' : '4px #FF3A54 solid', marginRight: '2px', filter: 'drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25))' }} className='lanePhaseChampImgBotLarge' src={`https://ddragon.leagueoflegends.com/cdn/${props.dataDragonVersion}/img/champion/${winnerChampName1}.png`}></img></a>
                        </div>
                    </Tooltip>
                    <Tooltip arrow placement='top' slotProps={{ popper: { modifiers: [{ name: 'offset', options: { offset: [0, -20] } }] } }} title={`${statsAt15.laneResults.BOTTOM.laneWinner[1]?.riotIdGameName} #${statsAt15.laneResults.BOTTOM.laneWinner[1]?.riotIdTagline}`}>
                        <div style={{ position: 'relative', display: 'inline-block' }}>
                            <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${statsAt15.laneResults.BOTTOM.laneWinner[1]?.riotIdGameName}/${statsAt15.laneResults.BOTTOM.laneWinner[1]?.riotIdTagline.toLowerCase()}`}><img style={{ border: statsAt15.laneResults.BOTTOM.laneWinner[1].teamId === 100 ? '4px #568CFF solid' : '4px #FF3A54 solid', filter: 'drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25))' }} className='lanePhaseChampImgBotLarge' src={`https://ddragon.leagueoflegends.com/cdn/${props.dataDragonVersion}/img/champion/${winnerChampName2}.png`}></img></a>
                        </div>
                    </Tooltip>
                    ...
                    <Tooltip arrow placement='top' slotProps={{ popper: { modifiers: [{ name: 'offset', options: { offset: [0, -20] } }] } }} title={`${statsAt15.laneResults.BOTTOM.laneLoser[0]?.riotIdGameName} #${statsAt15.laneResults.BOTTOM.laneLoser[0]?.riotIdTagline}`}>
                        <div style={{ position: 'relative', display: 'inline-block' }}>
                            <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${statsAt15.laneResults.BOTTOM.laneLoser[0]?.riotIdGameName}/${statsAt15.laneResults.BOTTOM.laneLoser[0]?.riotIdTagline.toLowerCase()}`}><img style={{ border: statsAt15.laneResults.BOTTOM.laneLoser[0].teamId === 100 ? '4px #568CFF solid' : '4px #FF3A54 solid', marginRight: '2px', filter: 'drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25))' }} className='lanePhaseChampImgBotLarge' src={`https://ddragon.leagueoflegends.com/cdn/${props.dataDragonVersion}/img/champion/${loserChampName1}.png`}></img></a>
                        </div>
                    </Tooltip>
                    <Tooltip arrow placement='top' slotProps={{ popper: { modifiers: [{ name: 'offset', options: { offset: [0, -20] } }] } }} title={`${statsAt15.laneResults.BOTTOM.laneLoser[1]?.riotIdGameName} #${statsAt15.laneResults.BOTTOM.laneLoser[1]?.riotIdTagline}`}>
                        <div style={{ position: 'relative', display: 'inline-block' }}>
                            <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${statsAt15.laneResults.BOTTOM.laneLoser[1]?.riotIdGameName}/${statsAt15.laneResults.BOTTOM.laneLoser[1]?.riotIdTagline.toLowerCase()}`}><img style={{ border: statsAt15.laneResults.BOTTOM.laneLoser[1].teamId === 100 ? '4px #568CFF solid' : '4px #FF3A54 solid', filter: 'drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25))' }} className='lanePhaseChampImgBotLarge' src={`https://ddragon.leagueoflegends.com/cdn/${props.dataDragonVersion}/img/champion/${loserChampName2}.png`}></img></a>
                        </div>
                    </Tooltip>
                    <img className='hideMobile hideTablet' style={{ margin: '20px', maxWidth: '65px', maxHeight: '65px', objectFit: 'contain' }} src='/images/laneIcons/Bottom.png'></img>
                </Grid>
            </Grid>
            ...
            <Grid className={lastButtonPressedBot === 'laneSumBot2' ? 'LanePhaseSummaryDetailsActive' : 'LanePhaseSummaryDetailsInActive'} style={{ flexDirection: 'row', display: 'flex' }}>
                    ...
                    <div style={{ marginBottom: '10px' }}>
                        {botLaneKillTimeline.map((kill, index) => {
                            if (kill.victimId !== 0 && kill.killerId !== 0) {
                                return (
                                    <Typography key={`kill_${index}`}>{Math.round(kill.timestamp / 60000)}m - <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${participants.find(player => player.participantId === kill.killerId)?.riotIdGameName}/${participants.find(player => player.participantId === kill.killerId)?.riotIdTagline.toLowerCase()}`} style={{ color: participants.find(player => player.participantId === kill.killerId).teamId === 100 ? '#0089D6' : '#FF1616', fontWeight: 'bold' }}>{participants.find(player => player.participantId === kill.killerId)?.riotIdGameName}</a><img style={{ maxWidth: '20px', maxHeight: '20px', marginLeft: '5px' }}
                                        src={`https://ddragon.leagueoflegends.com/cdn/${props.dataDragonVersion}/img/champion/${Object.values(champsJSON.data).find(champ => champ.key === String(participants.find(player => player.participantId === kill.killerId).championId)).id}.png`}></img> killed <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${participants.find(player => player.participantId === kill.victimId)?.riotIdGameName}/${participants.find(player => player.participantId === kill.victimId)?.riotIdTagline.toLowerCase()}`} style={{ color: participants.find(player => player.participantId === kill.victimId).teamId === 100 ? '#0089D6' : '#FF1616', fontWeight: 'bold' }}>{participants.find(player => player.participantId === kill.victimId)?.riotIdGameName}</a><img style={{ maxWidth: '20px', maxHeight: '20px', marginLeft: '5px' }} src={`https://ddragon.leagueoflegends.com/cdn/${props.dataDragonVersion}/img/champion/${Object.values(champsJSON.data).find(champ => champ.key === String(participants.find(player => player.participantId === kill.victimId).championId)).id}.png`}></img></Typography>
                                )
                            }
                            if (kill.victimId === 0) {
                                return (
                            ...
                            if (kill.killerId === 0) {
                                return (
                                    <Typography key={`kill_${index}`}>{Math.round(kill.timestamp / 60000)}m - <span style={{ fontWeight: 'bold', color: 'green' }}>Environment</span> killed <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${participants.find(player => player.participantId === kill.killerId)?.riotIdGameName}/${participants.find(player => player.participantId === kill.killerId)?.riotIdTagline.toLowerCase()}`} style={{ color: participants.find(player => player.participantId === kill.victimId).teamId === 100 ? '#0089D6' : '#FF1616', fontWeight: 'bold' }}>{participants.find(player => player.participantId === kill.victimId)?.riotIdGameName}</a><img style={{ maxWidth: '20px', maxHeight: '20px', marginLeft: '5px' }}
                ...
                <Grid item style={{ display: 'inline-flex', justifyContent: 'center' }} xs={12} sm={6}>
                    <Tooltip arrow placement='top' slotProps={{ popper: { modifiers: [{ name: 'offset', options: { offset: [0, -20] } }] } }} title={`${statsAt15.laneResults.BOTTOM.laneWinner[0]?.riotIdGameName} #${statsAt15.laneResults.BOTTOM.laneWinner[0]?.riotIdTagline}`}>
                        <div style={{ position: 'relative', display: 'inline-block' }}>
                            <a href={`/profile/${gameData.info.platformId.toLowerCase()}/${statsAt15.laneResults.BOTTOM.laneWinner[0]?.riotIdGameName}/${statsAt15.laneResults.BOTTOM.laneWinner[0]?.riotIdTagline.toLowerCase()}`}><img style={{ border: statsAt15.laneResults.BOTTOM.laneWinner[0].teamId === 100 ? '4px #568CFF solid' : '4px #FF3A54 solid', marginRight: '2px', filter: 'drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25))' }} className='lanePhaseChampImgBotLarge'
